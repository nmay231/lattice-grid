...
describe("partialMock", () => {
    ...
    it("fails to access a property the doesn't exist on the target", () => {
        ...
        expect(() => mock1.totally).toThrowErrorMatchingInlineSnapshot(
            `[Error: Could not access the property of { apple: 'asdf' }: \`.totally\`]`,
        );
        ...
        expect(() => mock3.totally.nested).toThrowErrorMatchingInlineSnapshot(
            `[Error: Could not access the property of { apple: 'asdf', totally: {} }: \`.totally.nested\`]`,
        );

        const mock4 = partialMock<ComplexType>({ totally: {} });
        expect(() => mock4.apple).toThrowErrorMatchingInlineSnapshot(
            `[Error: Could not access the property of { totally: {} }: \`.apple\`]`,
        );
    });
});
