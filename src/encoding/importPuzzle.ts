...
export const importPuzzleData = (
    ...
    for (const layer of data.layers) {
        ...
        // TODO: This is not correct since the answer objects should simply not be checked rather than checked to be nonexistent
...
export const extractPuzzleData = (inputText: string): LatestPuzzleData | ParseError => {
    const array = base64.parse(inputText);
    let data;
    try {
    ...
    if (data.uncompressedV1) {
        ...
        const grid = { type: "square" as const, ...squareGridParams };
        const result = extractLayersData(layers, grid);

        if ("internalMessage" in result) {
            return result;
        }
        nonfatalErrors.push(...result.nonfatalErrors);
        const outputLayers = result.layers;
...
export const extractLayersData = (
    layers: EncodedLayer[],
    squareGridParams: LatestPuzzleData["grid"],
): Pick<LatestPuzzleData, "layers" | "nonfatalErrors"> | ParseError => {
    // TODO: Will instantiating two grids (one here, and one for real) bite me in the butt later?
    const grid = new SquareGrid({ ...squareGridParams, minX: 0, minY: 0 });
    const encoder = grid.getEncoder({ cellSize: 2 }); // TODO: Hardcoded cellSize

    const nonfatalErrors = [] as ParseError[];
    ...
    for (const layerEnum of layers) {
        if (layerEnum.BackgroundColorLayer) {
            ...
            if (
                ...
                nonfatalErrors.push(
                    parseError("Internal error", message, { layers, squareGridParams }),
                );
                continue;
            }
    ...
    return { layers: outputLayers, nonfatalErrors };
};
